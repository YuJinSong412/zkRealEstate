...
template<typename FieldT>
void r1cs_constraint_system<FieldT>::swap_AB_if_beneficial( libff::profiling & profile )
{
    profile.enter_block("Call to r1cs_constraint_system::swap_AB_if_beneficial");

    profile.enter_block("Estimate densities");
    ...
    // {
        profile.print_indent(); profile_printf("* Non-zero A-count (estimate): %zu\n", non_zero_A_count);
        profile.print_indent(); profile_printf("* Non-zero B-count (estimate): %zu\n", non_zero_B_count);
    // }
    #endif
    
    profile.leave_block("Estimate densities");

    if (non_zero_B_count > non_zero_A_count)
    {
        profile.enter_block("Perform the swap");
        for (size_t i = 0; i < this->constraints.size(); ++i)
        {
            std::swap(this->constraints[i].a, this->constraints[i].b);
        }
        profile.leave_block("Perform the swap");
    }
    else
    {
        profile.print_indent(); printf("Swap is not beneficial, not performing\n");
    }

    profile.leave_block("Call to r1cs_constraint_system::swap_AB_if_beneficial");
}
