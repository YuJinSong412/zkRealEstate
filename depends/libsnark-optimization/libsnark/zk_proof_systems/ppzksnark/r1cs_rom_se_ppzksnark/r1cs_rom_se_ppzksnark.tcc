...
#ifndef R1CS_ROM_SE_PPZKSNARK_TCC_
#define R1CS_ROM_SE_PPZKSNARK_TCC_
...
bool r1cs_rom_se_ppzksnark_proving_key<ppT>::operator==(const r1cs_rom_se_ppzksnark_proving_key<ppT> &other) const
{
    return (
	this->g1 == other.g1 &&
	this->g2 == other.g2 &&
	this->alpha_g1 == other.alpha_g1 &&
            ...
            this->alpha_delta_g1 == other.alpha_delta_g1 &&
...
template<typename ppT>
bool r1cs_rom_se_ppzksnark_verification_key<ppT>::operator==(const r1cs_rom_se_ppzksnark_verification_key<ppT> &other) const
{
    return (
...
std::ostream& operator<<(std::ostream &out, const r1cs_rom_se_ppzksnark_verification_key<ppT> &vk)
{
    ...
    out << vk.g1 << OUTPUT_NEWLINE;
    out << vk.g2 << OUTPUT_NEWLINE;
...
template<typename ppT>
bool r1cs_rom_se_ppzksnark_processed_verification_key<ppT>::operator==(const r1cs_rom_se_ppzksnark_processed_verification_key<ppT> &other) const
{
    return (this->vk_alpha_g1_beta_g2 == other.vk_alpha_g1_beta_g2 &&
            this->vk_g2_precomp == other.vk_g2_precomp &&
...
std::ostream& operator<<(std::ostream &out, const r1cs_rom_se_ppzksnark_processed_verification_key<ppT> &pvk)
{
    ...
    out << pvk.vk_g1 << OUTPUT_NEWLINE;
    out << pvk.vk_delta_g2 << OUTPUT_NEWLINE;
    out << pvk.vk_g2_precomp << OUTPUT_NEWLINE;
...
template<typename ppT>
bool r1cs_rom_se_ppzksnark_proof<ppT>::operator==(const r1cs_rom_se_ppzksnark_proof<ppT> &other) const
{
...
template<typename ppT>
r1cs_rom_se_ppzksnark_verification_key<ppT> r1cs_rom_se_ppzksnark_verification_key<ppT>::dummy_verification_key(const size_t input_size)
{
    r1cs_rom_se_ppzksnark_verification_key<ppT> result;
    result.alpha_g1_beta_g2 = libff::Fr<ppT>::random_element() * libff::GT<ppT>::random_element();
    result.g1 = libff::G1<ppT>::random_element();
    result.g2 = libff::G2<ppT>::random_element();
...
template <typename ppT>
r1cs_rom_se_ppzksnark_keypair<ppT> r1cs_rom_se_ppzksnark_generator(const r1cs_rom_se_ppzksnark_constraint_system<ppT> &r1cs , libff::profiling & profile )
{
    profile.enter_block("Call to r1cs_rom_se_ppzksnark_generator");

    /* Make the B_query "lighter" if possible */
    r1cs_rom_se_ppzksnark_constraint_system<ppT> r1cs_copy(r1cs);
    ...
    /* The gamma inverse product component: (beta*A_i(t) + alpha*B_i(t) + gamma*C_i(t)) . */
    profile.enter_block("Compute gamma_ABC for R1CS verification key");
    libff::Fr_vector<ppT> gamma_ABC;
    gamma_ABC.reserve(qap.num_inputs());

    const libff::Fr<ppT> gamma_ABC_0 = (beta * At[0] + alpha * Bt[0] + gamma*Ct[0]);
    for (size_t i = 1; i < qap.num_inputs() + 1; ++i)
    {
        gamma_ABC.emplace_back((beta * At[i] + alpha * Bt[i] + gamma*Ct[i]));
    }
    profile.leave_block("Compute gamma_ABC for R1CS verification key");

    /* The delta inverse product component: gamma*(beta*A_i(t) + alpha*B_i(t) + gamma*C_i(t)) . */
    ...
    {
        Lt.emplace_back((beta * At[Lt_offset + i] + alpha * Bt[Lt_offset + i] + gamma * Ct[Lt_offset + i])*gamma );
    }
    ...
    const libff::G2<ppT> g2_generator = libff::G2<ppT>::random_element();
    ...
    knowledge_commitment_vector<libff::G1<ppT>, libff::G1<ppT> > A_query = kc_batch_exp(libff::Fr<ppT>::size_in_bits(), g1_window_size, g1_window_size, g1_table, g1_table, gamma, gamma * delta, At, chunks);
    ...
    knowledge_commitment_vector<libff::G2<ppT>, libff::G1<ppT> > B_query = kc_batch_exp(libff::Fr<ppT>::size_in_bits(), g2_window_size, g1_window_size, g2_table, g1_table, gamma, gamma, Bt, chunks);
    ...
    libff::G1_vector<ppT> H_query = batch_exp_with_coeff(g1_scalar_size, g1_window_size, g1_table, qap.Zt * gamma*gamma, Ht);
#ifdef USE_MIXED_ADDITION
    ...
    libff::G2<ppT> gamma_g2 = gamma * g2_generator;

    profile.enter_block("Encode gamma_ABC for R1CS verification key");
    ...
    profile.leave_block("Encode gamma_ABC for R1CS verification key");
    profile.leave_block("Generate R1CS verification key");

    profile.leave_block("Call to r1cs_rom_se_ppzksnark_generator");

    accumulation_vector<libff::G1<ppT> > gamma_ABC_g1(std::move(gamma_ABC_g1_0), std::move(gamma_ABC_g1_values));

    r1cs_rom_se_ppzksnark_verification_key<ppT> vk = r1cs_rom_se_ppzksnark_verification_key<ppT>(g1,
		    									g2,
		    									alpha_g1_beta_g2,
                                                                                         gamma_g2,
                                                                                         delta_g2,
    ...
    r1cs_rom_se_ppzksnark_proving_key<ppT> pk = r1cs_rom_se_ppzksnark_proving_key<ppT>(
										std::move(g1),
										std::move(g2),
										std::move(alpha_g1),
                                                                               std::move(beta_g1),
                                                                               std::move(delta_g1),
                                                                               std::move(alpha_delta_g1),
                                                                               std::move(beta_g2),
                                                                               std::move(delta_g2),
                                                                               std::move(A_query),
                                                                               std::move(B_query),
    ...
    return r1cs_rom_se_ppzksnark_keypair<ppT>(std::move(pk), std::move(vk));
}
...
{
    profile.enter_block("Call to r1cs_rom_se_ppzksnark_prover");
    ...
    knowledge_commitment<libff::G1<ppT>, libff::G1<ppT> > evaluation_At = kc_multi_exp_with_mixed_addition<libff::G1<ppT>,
                                                                                                           libff::G1<ppT>,
                                                                                                           libff::Fr<ppT>,
    ...
    /* A = alpha + sum_i(a_i*A_i(t)) + r */
    libff::G1<ppT> g1_A = pk.alpha_g1 + evaluation_At.g + r * pk.g1;
    libff::G1<ppT> g1_A_delta = pk.alpha_delta_g1 + evaluation_At.h + r * pk.delta_g1;

    /* B = beta + sum_i(a_i*B_i(t)) + s */
    libff::G1<ppT> g1_B = pk.beta_g1 + evaluation_Bt.h + s * pk.g1;
    libff::G2<ppT> g2_B = pk.beta_g2 + evaluation_Bt.g + s * pk.g2;

    /* C = sum_i(a_i*(gamma*(beta*A_i(t) + alpha*B_i(t) + gamma*C_i(t)) + gamma^2*H(t)*Z(t) + a*s + r*b - r*s  + delta*a*h_2 + b*h_1 + delta*h_1*h_2*/
    const libff::Fr<ppT> h1 = hash<ppT>(g1_A, g2_B, 1);
    const libff::Fr<ppT> h2 = hash<ppT>(g1_A, g2_B, 2);
    libff::G1<ppT> g1_C = evaluation_Ht + evaluation_Lt + s *  g1_A + r * g1_B - (r * s) * pk.g1 + h2 * g1_A_delta + h1 * g1_B + h1 * h2 * pk.delta_g1;

    profile.leave_block("Compute the proof");

    profile.leave_block("Call to r1cs_rom_se_ppzksnark_prover");

    r1cs_rom_se_ppzksnark_proof<ppT> proof = r1cs_rom_se_ppzksnark_proof<ppT>(std::move(g1_A), std::move(g2_B), std::move(g1_C));
    proof.print_size();
...
template <typename ppT>
r1cs_rom_se_ppzksnark_processed_verification_key<ppT> r1cs_rom_se_ppzksnark_verifier_process_vk(const r1cs_rom_se_ppzksnark_verification_key<ppT> &vk , libff::profiling & profile)
{
    profile.enter_block("Call to r1cs_rom_se_ppzksnark_verifier_process_vk");

    r1cs_rom_se_ppzksnark_processed_verification_key<ppT> pvk;
    pvk.vk_alpha_g1_beta_g2 = vk.alpha_g1_beta_g2;
    pvk.vk_g1 = vk.g1;
    pvk.vk_delta_g2 = vk.delta_g2;
    pvk.vk_g2_precomp = ppT::precompute_G2(vk.g2);
    ...
    profile.leave_block("Call to r1cs_rom_se_ppzksnark_verifier_process_vk");
...
bool r1cs_rom_se_ppzksnark_online_verifier_weak_IC(const r1cs_rom_se_ppzksnark_processed_verification_key<ppT> &pvk,
                                               ...
                                               libff::profiling & profile)
{
    profile.enter_block("Call to r1cs_rom_se_ppzksnark_online_verifier_weak_IC");
    ...
    // h1 and h2 are hash values of (A,B).
    const libff::Fr<ppT> h1 = hash<ppT>(proof.g_A, proof.g_B, 1);
    const libff::Fr<ppT> h2 = hash<ppT>(proof.g_A, proof.g_B, 2);

    const libff::G1_precomp<ppT> proof_g_A_precomp = ppT::precompute_G1(proof.g_A + h1 * pvk.vk_g1);
    const libff::G2_precomp<ppT> proof_g_B_precomp = ppT::precompute_G2(proof.g_B + h2 * pvk.vk_delta_g2);
    ...
    const libff::Fqk<ppT> QAP2 = ppT::double_miller_loop(
        ...
        proof_g_C_precomp, pvk.vk_g2_precomp);
    ...
    profile.leave_block("Call to r1cs_rom_se_ppzksnark_online_verifier_weak_IC");
...
bool r1cs_rom_se_ppzksnark_verifier_weak_IC(const r1cs_rom_se_ppzksnark_verification_key<ppT> &vk,
                                        ...
                                        libff::profiling & profile)
{
    profile.enter_block("Call to r1cs_rom_se_ppzksnark_verifier_weak_IC");
    r1cs_rom_se_ppzksnark_processed_verification_key<ppT> pvk = r1cs_rom_se_ppzksnark_verifier_process_vk<ppT>(vk);
    bool result = r1cs_rom_se_ppzksnark_online_verifier_weak_IC<ppT>(pvk, primary_input, proof);
    profile.leave_block("Call to r1cs_rom_se_ppzksnark_verifier_weak_IC");
    return result;
}
...
{
    bool result = true;
    profile.enter_block("Call to r1cs_rom_se_ppzksnark_online_verifier_strong_IC");
    ...
    {
        result = r1cs_rom_se_ppzksnark_online_verifier_weak_IC(pvk, primary_input, proof, profile );
    }

    profile.leave_block("Call to r1cs_rom_se_ppzksnark_online_verifier_strong_IC");
    return result;
}
...
bool r1cs_rom_se_ppzksnark_verifier_strong_IC(const r1cs_rom_se_ppzksnark_verification_key<ppT> &vk,
                                          ...
                                          libff::profiling & profile)
{
    profile.enter_block("Call to r1cs_rom_se_ppzksnark_verifier_strong_IC");
    r1cs_rom_se_ppzksnark_processed_verification_key<ppT> pvk = r1cs_rom_se_ppzksnark_verifier_process_vk<ppT>(vk, profile);
    bool result = r1cs_rom_se_ppzksnark_online_verifier_strong_IC<ppT>(pvk, primary_input, proof, profile);
    profile.leave_block("Call to r1cs_rom_se_ppzksnark_verifier_strong_IC");
    return result;
}
...
bool r1cs_rom_se_ppzksnark_affine_verifier_weak_IC(const r1cs_rom_se_ppzksnark_verification_key<ppT> &vk,
                                               ...
                                               libff::profiling & profile)
{
    profile.enter_block("Call to r1cs_rom_se_ppzksnark_affine_verifier_weak_IC");
    ...
    libff::affine_ate_G2_precomp<ppT> pvk_vk_g2_precomp = ppT::affine_ate_precompute_G2(vk.g2);
    ...
    const libff::affine_ate_G1_precomp<ppT> proof_g_A_precomp = ppT::affine_ate_precompute_G1(proof.g_A + h1 * vk.g1);
    const libff::affine_ate_G2_precomp<ppT> proof_g_B_precomp = ppT::affine_ate_precompute_G2(proof.g_B + h2 * vk.delta_g2);
    ...
    const libff::Fqk<ppT> QAP_miller = ppT::affine_ate_e_times_e_over_e_miller_loop(
        ...
        proof_g_C_precomp, pvk_vk_g2_precomp,
    ...
    profile.leave_block("Call to r1cs_rom_se_ppzksnark_affine_verifier_weak_IC");
...
libff::Fr<ppT> hash(const libff::G1<ppT> g1_a, const libff::G2<ppT> g2_b, const int type)
{
	...
	SHA256((unsigned char*)result.str().c_str(), result.str().length(), (unsigned char*)&digest);

	digest[SHA256_DIGEST_LENGTH]=0;

	libff::Fr<ppT> hash_result;
	std::stringstream digestResult;
	digestResult << digest; 
	digestResult >> hash_result;
...
#endif // R1CS_ROM_SE_PPZKSNARK_TCC_
