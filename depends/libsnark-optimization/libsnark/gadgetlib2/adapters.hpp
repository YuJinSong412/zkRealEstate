...
public:
    ...
    linear_term_t convert(const LinearTerm<FieldT>& lt) const;
    linear_combination_t convert(const LinearCombination<FieldT>& lc) const;
    constraint_t convert(const Constraint<FieldT>& constraint) const;
    constraint_sys_t convert(const ConstraintSystem<FieldT>& constraint_sys) const;
    assignment_t convert(const VariableAssignment<FieldT>& assignment) const;
    ...
    protoboard_t convert(const Protoboard<FieldT>& pb) const;
    Fp_elem_t convert(FElem<FieldT> fElem) const;
    // static size_t getVariableIndex(const Variable& v){return v.index();}
};
