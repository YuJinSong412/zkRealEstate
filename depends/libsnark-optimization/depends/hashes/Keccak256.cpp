...
namespace Hashes {
    ...
    std::vector<uint8_t> keccak256(std::vector<uint8_t> & _input){
        ...
        _keccak256_hash_(output.data(), output.size() , _input.data(), _input.size(), 200 - (256 / 4), 0x01);
        return output;
    }

    std::vector<uint8_t> keccak256( uint8_t *_input , size_t _input_size ){
        vector<uint8_t> output(32) ;
        _keccak256_hash_(output.data(), output.size() , _input , _input_size , 200 - (256 / 4), 0x01);
        return output;
    }
