...
TEST_TXS_FILE_PATH = TEST_FILE_PATH + "/txs"
...
class Scenario(TestCase) :
    def Scenario(self, env, depth, hash, zksnark, ec, scope):
        ...
        for i in range(scope[0], scope[1]):
            ...
            if exists(file_path) :
                trans_param = TransParameters.load_trans_param(TEST_TX_FILE_NAME + str(i) + ".json", TEST_TXS_FILE_PATH)
            else :
                os.chdir(TEST_FILE_PATH + DEFAULT_DEPLOYER)
                trans_param, _ = zklay_client.create_trans_args(
                ...
                os.chdir(cur_pwd)
                trans_param.write_trans_param(TEST_TX_FILE_NAME + str(i) + ".json", TEST_TXS_FILE_PATH)
            trans_param = TransParameters.load_trans_param(TEST_TX_FILE_NAME + str(i) + ".json", TEST_TXS_FILE_PATH)
           
            tx_hash = zklay_client.anontransfer(trans_param, alice_eth_address, alice_eth_priv_key, pocket.v_in)
...
if __name__ == "__main__" :
    ...
    ec = constants.ELLIPTIC_CURVE_BN256
    scope = [0,10]
